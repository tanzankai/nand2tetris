...
JackTokenizer *tokenizerInit(FILE *fp_jack){
    ...
    tokenizer.fp_jack = fp_jack;
    fseek(tokenizer.fp_jack, 0, SEEK_SET);
    skip(tokenizer.fp_jack);
...
bool hasMoreTokens(JackTokenizer *tokenizer){
    return !isEOF(tokenizer->fp_jack);
}

void advance(JackTokenizer *tokenizer){
    if(getSymbol(tokenizer->fp_jack, tokenizer->token)) tokenizer->tokenkind = TK_SYMBOL;
    else if(getIntConst(tokenizer->fp_jack, tokenizer->token)) tokenizer->tokenkind = TK_INTCONST;
    else if(getStringConst(tokenizer->fp_jack, tokenizer->token)) tokenizer->tokenkind = TK_STRCONST;
    else{
        getKeywordOrIdentifier(tokenizer->fp_jack, tokenizer->token);
